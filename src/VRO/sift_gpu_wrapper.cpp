...
SiftGPUWrapper::SiftGPUWrapper() {
   ...
   // changed by David Z Oct.14, 2015, for changing the parameters of sift gpu
    // ros::NodeHandle nh("~");
    double edge_threshold_parameter, contrastThreshold;
    int max_features; 
    // nh.param<double>("siftgpu_edge_threshold", edge_threshold_parameter, 10.);  // 10.
    // nh.param<double>("siftgpu_contrast_threshold", contrastThreshold, 0.0008); // 0.0008 0.02/3.
    // nh.param<int>("max_keypoints", max_features, 500);
    CParams* pP = CParams::Instance(); 
    edge_threshold_parameter = pP->m_siftgpu_edge_threshold; 
    contrastThreshold = pP->m_siftgpu_contrast_threshold; 
    max_features = pP->m_max_num_features;
    ...
    char DOG_threshold_value[10]; sprintf(DOG_threshold_value, "%f", contrastThreshold);

    char edge_threshold[] = "-e"; 
    // char edge_threshold_val[] = "20.0"; // try to incresae the threshold"10.0"; //Default: 10
    char edge_threshold_val[10];  sprintf(edge_threshold_val, "%f", edge_threshold_parameter);

    char dog_levels[] = "-d"; //This parameter affects the number of keypoints found
    char dog_levels_val[] = "7"; // 7 5 Default: 3 (would result in less features for, e.g., blurry images)
    ...
    char * argv[] = {method,  
                     ...
                     nonfixedorientation,
                     DOG_threshold, DOG_threshold_value};

    // siftgpu->ParseParam(15, argv);
    siftgpu->ParseParam(17, argv);
